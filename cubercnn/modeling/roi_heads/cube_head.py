...
class CubeHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.use_prior          = cfg.MODEL.ROI_CUBE_HEAD.DIMS_PRIORS_ENABLED
        #-------------------------------------------
        # Feature generator
        #-------------------------------------------
        ...
        # 3D outputs
        #-------------------------------------------
        output_multiple_factor = self.num_classes if self.use_prior else 1
        # Dimensions in meters (width, height, length)
        self.bbox_3D_dims = nn.Linear(self._output_size, output_multiple_factor*3)
        ...
        # XY
        self.bbox_3D_center_deltas = nn.Linear(self._output_size, output_multiple_factor*2)
        ...
        if self.pose_type == '6d':
            self.bbox_3D_pose = nn.Linear(self._output_size, output_multiple_factor*6)

        elif self.pose_type == 'quaternion':
            self.bbox_3D_pose = nn.Linear(self._output_size, output_multiple_factor*4)

        elif self.pose_type == 'euler':
            self.bbox_3D_pose = nn.Linear(self._output_size, output_multiple_factor*3)
        ...
        # Z 
        self.bbox_3D_center_depth = nn.Linear(self._output_size, output_multiple_factor*cluster_bins)
        ...
        if self.use_conf:
            self.bbox_3D_uncertainty = nn.Linear(self._output_size, output_multiple_factor*1)
    ...
    def forward(self, x, num_boxes_per_image: Optional[List[int]] = None):
        ...
        if self.cluster_bins > 1:
            if self.use_prior:
                ...
                box_z = box_z.view(n, self.cluster_bins, -1)
